...
function parseArgs(argv) {
  ...
  for (const a of argv) {
    if (a.startsWith('--')) {
      ...
      else if (k === 'width' || k === 'height' || k === 'fps' || k === 'tiers') opts[k] = parseInt(v, 10);
...
function runFfmpeg(args) {
  return new Promise((resolve, reject) => {
    ...
    ff.on('error', (e) => {
      if (e.code === 'ENOENT') {
        reject(new Error('ffmpeg not found on PATH. Install with `brew install ffmpeg`.'));
      } else {
        reject(e);
      }
    });
...
function frameToAscii(buf, offset, width, height, ramp, invert, tiers) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      let g = ((a + b) / 2) / 255;
      if (invert) g = 1 - g;
      const idx = Math.max(0, Math.min(rampLen - 1, Math.floor(g * (rampLen - 1) + 0.0001)));
...
function emitModule({ output, name, frames, meta }) {
  const banner =
`// Auto-generated by scripts/ascii-video-convert.mjs
// Source: ${meta.input}
...
async function main() {
  ...
  if (!opts.input || !opts.output) {
    console.error('Usage: node scripts/ascii-video-convert.mjs <input> <output.js> [options]');
    console.error('See file header for options.');
    process.exit(1);
  }

  const { input, output, width, height, fps, invert, ramp, name, start, duration, tiers } = opts;
  const srcH = height * 2;
  ...
  console.log(`[ascii-video] reading ${input} → ${width}×${srcH} grayscale @ ${fps}fps`);
  const buf = await runFfmpeg(ffArgs);

  const bytesPerFrame = width * srcH;
  const numFrames = Math.floor(buf.length / bytesPerFrame);
  console.log(`[ascii-video] got ${numFrames} frames (${(buf.length / 1024).toFixed(1)} KB raw)`);
  ...
  for (let f = 0; f < numFrames; f++) {
    frames.push(frameToAscii(buf, f * bytesPerFrame, width, height, ramp, invert, tiers));
  }
  ...
  console.log(`[ascii-video] wrote ${output} (${numFrames} frames, ~${(outBytes / 1024).toFixed(1)} KB uncompressed)`);
}
